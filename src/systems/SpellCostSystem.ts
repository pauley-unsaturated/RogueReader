...
export class SpellCostSystem {
   ...
   *
   * Item #15: Timer delayed to 4th/5th grade (Floors 11+) for less stress on early readers
   */
  static calculateSpellCost(config: SpellCostConfig): SpellAttemptConfig {
    const { currentFloor, playerMP } = config;

    // K-3rd Grade (Floors 1-10): Tries Mode ONLY (no stressful timer for early readers)
    if (currentFloor <= 10) {
      const mpPerSpell = 10;
    ...
    // 4th/5th+ Grade (Floors 11+): Timer Mode (introduces time pressure for advanced readers)
    const mpPerSpell = 15;
